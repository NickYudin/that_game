...
RSpec.describe EpicBattle, type: :model do
  describe '#call' do
    ...
    it 'initiative method should create array of fighters' do
      expect(EpicBattle.new(character, monster).initiative.class).to eq(Array)
    end

    it 'counting initiative modificator correctly' do
      expect(EpicBattle.new(character, monster).initiative_modificator(character)).to eq(5)
      expect(EpicBattle.new(character, monster).initiative_modificator(monster)).to eq(-20)
    end

    it 'initiative_of works correctly' do
      100.times do
        expect(EpicBattle.new(character, monster).initiative_of(character)).to be_between(6, 25).inclusive
      end
    end
    ...
    it 'order method works correctly' do
      expect(EpicBattle.new(character, monster).order).to eq(monster)
    end
  end
end
